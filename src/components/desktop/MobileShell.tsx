...
import { AppFrameContext } from '../adw/frame';
...
export function MobileShell({ initialApp }: MobileShellProps) {
  ...
  useEffect(() => {
    ...
    if (active) section.current?.focus({ preventScroll: true });
  }, [active]);
      ...
      <AnimatePresence mode="wait" initial={false}>
        {active ? (
          <motion.section
            ...
            aria-label={getApp(active.id).title}
            tabIndex={-1}
          ...
          >
            {/* libadwaita apps draw their own header bar; the terminal gets a plain one. */}
            {getApp(active.id).chrome && (
              <header className={styles.appBar}>
                <button type="button" className={styles.back} onClick={goHome} aria-label="Back to home">
                  <Icon name="back" size={18} />
                </button>
                <h2 className={styles.appTitle}>{getApp(active.id).title}</h2>
              </header>
            )}
            <div className={styles.appBody} data-chrome={getApp(active.id).chrome ?? 'adwaita'}>
              <AppFrameContext.Provider value={{ kind: 'mobile', appTitle: getApp(active.id).title, goHome }}>
